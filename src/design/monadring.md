...
|---------------------------------------------|-------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Substrate Off-chain Worker                  | Validator as an Oracle. Initiating IO requests then submit the response as a normal transaction to the consensus layer. | Can’t be verified. Limited data i/o activeness.                                                                                                                                                                                   |
| ICP                                         | All nodes within a subnet initiate the same request then compare the result.                                            | Can’t be used for non-idempotent servers （i.e  submitting blockchain transactions, accessing LLMs)                                                                                                                               |
| Regular Blockchain (passive) + zkTLS Oracle | Deploy a zk-prover to prove the TLS handshake and then submit to a smart contract.                                      | The zk-prover needs to pay for the gas hence much less motivated                                                                                                                                                                  |
| Verisense                                   | One validator sends the request and others verify it by TLS handshaking.                                                | Overcome all the above limitations. Response delay is acceptable for most use cases (post a picture, post a tweet, response in TG and etc). Improvement of data i/o response is designed and will be in place in later iteration. |
